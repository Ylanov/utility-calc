...
SETTINGS_SEED = [
    ...
    # ---- Финансовые правила (вкладка «Финансовая отчётность») ----
    ("finance.debt_growing.enabled", "true", "bool", "finance",
     "Долг растёт 3+ месяца подряд → флаг DEBT_GROWING.",
     None, None, True),
    ("finance.bill_spike.enabled", "true", "bool", "finance",
     "Сумма квитанции выросла >50% относительно прошлого месяца.",
     None, None, True),
    ("finance.bill_spike.threshold_pct", "50", "int", "finance",
     "Порог в процентах для BILL_SPIKE.", "10", "300", True),
    ("finance.bill_drop.enabled", "true", "bool", "finance",
     "Сумма упала >50% — возможно человек не подал ничего за период.",
     None, None, True),
    ("finance.bill_drop.threshold_pct", "50", "int", "finance",
     "Порог в процентах для BILL_DROP.", "10", "100", True),
    ("finance.zero_bill.enabled", "true", "bool", "finance",
     "Нулевая квитанция при ненулевой истории — что-то не так.",
     None, None, True),
    ("finance.overpay_suspect.enabled", "true", "bool", "finance",
     "Большая переплата (>10000 ₽) — может быть ошибкой импорта из 1С.",
     None, None, True),
    ("finance.overpay_suspect.threshold_rub", "10000", "int", "finance",
     "Сумма в рублях, выше которой переплата считается подозрительной.",
     "0", "1000000", True),
    ("finance.high_bill_per_person.enabled", "true", "bool", "finance",
     "Счёт на одного проживающего больше N₽ — нетипично.",
     None, None, True),
    ("finance.high_bill_per_person.threshold_rub", "8000", "int", "finance",
     "Порог в рублях на 1 проживающего.", "1000", "100000", True),
