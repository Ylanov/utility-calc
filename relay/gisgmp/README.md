...
- Долг считается за окно `months_back` мес. Долги старше окна не учитываются
  ...
  (для хронических должников нужно большое окно, иначе старые неоплаченные не попадут).

## Диагностика по плательщику — `check_payer.py`

Если в реестре и в ЖКХ суммы расходятся — `check_payer.py` показывает ПО СТРОКАМ
все начисления одного человека и как складывается долг 205/209 (что в долг, что
пропущено как оплаченное/аннулированное). Запуск на ВМ PODS2:

```bash
sudo python3 /opt/gisgmp-relay/check_payer.py "Фамилия"
```

Показывает ВСЕ месяцы (без окна) — удобно сверять полный долг против того, что
дал релей с окном. Креды берёт из `relay.env` (нужен sudo).
