...
def build_daily_briefing_prompt(metrics: dict) -> list[dict]:
    ...
    metrics = {
        ...
        "anomaly_readings_yesterday": int,     # reading'и высокого риска (score>=80)
        "format_suspect_yesterday": int,       # показания >99999 (битый формат)
        "high_cost_readings_yesterday": [...], # квитанции >30k ₽ (вероятно баги)
        "top_errors": [...],                   # последние ошибки копилки (для разбора+решения)
    }
    """
    system = SYSTEM_BASE + (
        " Сейчас ты делаешь ежедневную утреннюю сводку для админа: "
        "коротко (200-400 слов), MARKDOWN, разделы: «📊 Цифры за вчера», "
        "«⚠ На что обратить внимание», «🛠 Ошибки и что делать» (если есть "
        "top_errors), «✅ Хорошие новости» (если есть). "
        "ВАЖНО про метрики: anomaly_readings_yesterday — это показания с "
        "ВЫСОКИМ риском (score≥80), требующие ручной проверки; это НЕ все "
        "показания. new_readings_created — сколько подано всего; не называй их "
        "аномалиями. Если anomaly_readings_yesterday / format_suspect_yesterday "
        "/ high_cost_readings_yesterday > 0 — вынеси в «На что обратить "
        "внимание» с конкретными цифрами. "
        "В разделе «🛠 Ошибки и что делать» по КАЖДОЙ записи top_errors дай: "
        "1) что произошло простыми словами (по path/status/message), "
        "2) вероятную причину, 3) КОНКРЕТНЫЙ шаг решения для админа. Группируй "
        "одинаковые ошибки. Если ошибка похожа на действие пользователя "
        "(валидация 400/422) — так и скажи, без паники. "
        "Без эмодзи-избытка. Без воды. Если совсем ничего важного "
        "(все нули и нет top_errors) — пиши одной строкой «Всё спокойно»."
    )
    user = (
