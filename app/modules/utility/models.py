...
class Room(Base):
    ...
    # Постоянный неугадываемый токен квартиры для анонимного QR-портала
    # подачи показаний (ссылка /q/<qr_token>). Выдаётся лениво при первой
    # генерации QR в админке (secrets.token_urlsafe(32)). UNIQUE → резолв
    # комнаты по токену O(1). Отзыв = перегенерация (старый QR протухает).
    qr_token = Column(String, unique=True, index=True, nullable=True)

    # Пароль QR-портала (argon2-хеш). Второй фактор к токену: QR могут
    # сфотографировать (гость/соседи) — пароль знает только жилец. NULL =
    # ещё не установлен → портал при первом входе просит придумать пароль.
    # Сброс: админ в модалке QR или перевыпуск токена.
    qr_password_hash = Column(String, nullable=True)
