...
async def bulk_approve_readings(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    return await admin_readings_service.bulk_approve_drafts(db)

@router.post("/api/admin/approve/{reading_id}")
async def approve_reading(reading_id: int, correction_data: ApproveRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    check_role(current_user, ["accountant", "admin"])
    return await admin_readings_service.approve_single(db, reading_id, correction_data)

@router.delete("/api/admin/readings/{reading_id}")
async def delete_reading_record(reading_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    check_role(current_user, ["accountant", "admin"])
    return await admin_readings_service.delete_reading(db, reading_id)

@router.get("/api/admin/readings/manual-state/{user_id}")
async def get_manual_reading_state(user_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    check_role(current_user, ["accountant", "admin"])
    return await admin_readings_service.get_manual_state(db, user_id)

@router.post("/api/admin/readings/manual")
async def save_manual_reading(data: AdminManualReadingSchema, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    check_role(current_user, ["accountant", "admin"])
    return await admin_readings_service.save_manual_entry(db, data)

@router.post("/api/admin/readings/one-time")
async def create_one_time_charge(data: OneTimeChargeSchema, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    check_role(current_user, ["accountant", "admin"])
    return await admin_readings_service.create_one_time_charge(db, data)
