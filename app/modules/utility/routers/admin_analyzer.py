...
async def ai_triage_reading(
    ...
    """ИИ-триаж подозрительного показания через GigaChat. By-request (по
    кнопке) — бережёт Freemium-бюджет. Собирает контекст reading'а (показания,
    история, тариф, соседи с тем же baseline, долг) и просит ИИ определить
    вероятную причину аномалии + рекомендацию админу.
    """
    _require_admin(current_user)
    from decimal import Decimal as _D
    import json as _json
    from app.modules.llm import service as llm_service
    from app.modules.llm.prompts import build_anomaly_triage_prompt

    # Понятная 409 вместо 500, если ИИ не настроен/выключен/бюджет исчерпан.
    ok, reason = await llm_service.is_available(db)
    if not ok:
        raise HTTPException(status_code=409, detail=f"ИИ недоступен: {reason}")
    ...
    if not reading:
        raise HTTPException(status_code=404, detail="Reading не найден")

    user = reading.user
    room = user.room if user else None

    # История: последние 6 approved этого user+room (без текущего).
    ...
    d_hot = float((reading.hot_water or _D("0")) - (prev.hot_water if prev and prev.hot_water else _D("0")))
    d_cold = float((reading.cold_water or _D("0")) - (prev.cold_water if prev and prev.cold_water else _D("0")))

    # Сколько жильцов имеют ТО ЖЕ предыдущее показание (cloned-baseline сигнал).
    neighbors_same = 0
    if prev and (prev.hot_water or prev.cold_water):
        neighbors_same = int((await db.execute(
            select(func.count(func.distinct(MeterReading.user_id)))
            .where(
                MeterReading.period_id == prev.period_id,
                MeterReading.hot_water == prev.hot_water,
                MeterReading.cold_water == prev.cold_water,
                MeterReading.is_approved.is_(True),
            )
        )).scalar() or 0)
    ...
    context = {
        "reading": {
            "id": reading.id,
            "period": reading.period.name if reading.period else None,
        ...
        "prev_reading": ({
            "period": prev.period.name if prev and prev.period else None,
            "hot_water": float(prev.hot_water or 0),
            "cold_water": float(prev.cold_water or 0),
        } if prev else None),
        "delta": {"hot": d_hot, "cold": d_cold},
        "user": {
            "username": user.username if user else None,
            "residents_count": getattr(user, "residents_count", None),
            "billing_mode": getattr(user, "billing_mode", None),
        },
        "room": {
            "address": room.format_address if room else None,
            "area": float(getattr(room, "apartment_area", 0) or 0) if room else None,
            "dormitory": room.dormitory_name if room else None,
        },
        "tariff": {
            "name": tariff.name if tariff else None,
            "water_supply": float(getattr(tariff, "water_supply", 0) or 0) if tariff else None,
            "water_heating": float(getattr(tariff, "water_heating", 0) or 0) if tariff else None,
        },
        "history": [
            {
                "period": h.period.name if h.period else None,
                "hot_water": float(h.hot_water or 0),
                "cold_water": float(h.cold_water or 0),
                "total_cost": float(h.total_cost or 0),
            } for h in history
        ],
        "neighbors_with_same_prev_value": neighbors_same,
        "debt": {
            "debt_209": float(reading.debt_209 or 0),
            "debt_205": float(reading.debt_205 or 0),
            "overpayment_209": float(reading.overpayment_209 or 0),
            "overpayment_205": float(reading.overpayment_205 or 0),
        },
        "format_suspect": bool(
            (reading.hot_water or 0) > 99999 or (reading.cold_water or 0) > 99999
        ),
    }

    messages = build_anomaly_triage_prompt(context)
    result = await llm_service.ask(
        db, messages,
        purpose="anomaly_triage",
        related_type="reading",
        related_id=reading_id,
        temperature=0.2,
        max_tokens=700,
        timeout=60,
    )
    if not result.ok:
        raise HTTPException(status_code=502, detail=f"ИИ-ошибка: {result.error}")

    # Парсим JSON-вердикт. ИИ иногда оборачивает в ```json — чистим.
    ...
    return {
        ...
        "verdict": verdict,
        "cost_rub": float(result.cost_rub or 0),
