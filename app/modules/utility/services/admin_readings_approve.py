...
async def approve_single(db: AsyncSession, reading_id: int, correction_data: ApproveRequest, current_user=None):
    ...
    # ─────────────────────────────────────────────────────────────
    # RETROACTIVE RECALC после возврата жильца (см. skip_recalc.py).
    # Если перед этим reading-ом была цепочка AUTO_AVG / AUTO_NORM_SANCTION
    # (жилец пропустил 1+ месяцев), пересчитываем эти прошлые auto-reading'и
    # равномерно по реальной дельте. Иначе помесячно получается «лесенка»:
    # auto-месяц занижен (или завышен), а текущий — «выровнен» с большим cost.
    # См. инцидент с возвратом жильца май 2026.
    # ─────────────────────────────────────────────────────────────
    if not is_baseline:
