...
async def approve_single(db: AsyncSession, reading_id: int, correction_data: ApproveRequest, current_user=None):
    ...
    # Коротко: юзер хочет чтобы auto-readings сохраняли norm × residents,
    # а manual подача считалась обычной delta от prev (включая virtual).
    # Сторно volume-cost больше не нужен.
