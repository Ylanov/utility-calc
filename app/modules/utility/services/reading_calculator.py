...
# (find_chronological_prev_reading_sync / find_meaningful_prev_reading_sync
#  удалены 2026-07-14: вызовов не осталось, а их period_id-семантика ломалась
#  на ретроактивных периодах. Канон — pick_prev_pair / find_prev_reading ниже.)
...
# =====================================================================
# ЕДИНЫЙ ВЫБОР PREV (аудит 2026-07-14): раньше жило 5+ реализаций
# (approve_single/bulk — по period_id; ручной ввод/квитанция/карточка —
# по chron_key имени периода; клиентская подача — по period_id). При
# ретроактивных периодах (id не отражает месяц) они выбирали РАЗНЫЕ prev
# → разные суммы на разных путях. Канон: хронология по ИМЕНИ периода
# (period_chron_key) + is_meaningful_prev + приоритет METER_REPLACEMENT
# текущего месяца (новый baseline после замены счётчика).
# =====================================================================

def pick_prev_pair(rows, target_period_name):
    """Чистый выбор prev из кандидатов. rows: iterable[(reading, period_name)].

    Возвращает (prev_meaningful, prev_any, earlier_desc):
      prev_meaningful — «прошлое» для расчёта дельты: сначала METER_REPLACEMENT
                        ТЕКУЩЕГО месяца (baseline нового счётчика), затем самое
                        свежее meaningful из СТРОГО более ранних месяцев;
      prev_any        — самое свежее из более ранних месяцев БЕЗ фильтра
                        meaningful (для synth-baseline-детекции);
      earlier_desc    — все показания более ранних месяцев, свежие первыми
                        (история для анализатора аномалий).
    Тайбрейк внутри месяца — created_at (свежее первым).
    """
    tkey = period_chron_key(target_period_name)
    ...
    earlier, replacement = [], []
    for r, nm in rows:
        k = period_chron_key(nm)
        if k < tkey:
            earlier.append((k, r))
        elif k == tkey and "METER_REPLACEMENT" in (getattr(r, "anomaly_flags", None) or ""):
            replacement.append((k, r))
    earlier.sort(key=lambda x: (x[0], _ckey(x[1])), reverse=True)
    replacement.sort(key=lambda x: _ckey(x[1]), reverse=True)

    earlier_desc = [r for _, r in earlier]
    ordered = [r for _, r in replacement] + earlier_desc
    prev_meaningful = next((r for r in ordered if is_meaningful_prev(r)), None)
    prev_any = earlier_desc[0] if earlier_desc else None
...
async def find_prev_reading(db, *, user_id, room_id, target_period_name,
    ...
    КОМНАТЕ (кроме exclude_id), период задан → pick_prev_pair.

    Возвращает ту же тройку, что pick_prev_pair.
    """
    from sqlalchemy import select as _select
    from app.modules.utility.models import BillingPeriod as _BP

    q = (
        _select(MeterReading, _BP.name)
        .join(_BP, _BP.id == MeterReading.period_id)
        .where(
    ...
    if exclude_id is not None:
        q = q.where(MeterReading.id != exclude_id)
    rows = (await db.execute(q)).all()
    return pick_prev_pair(rows, target_period_name)
