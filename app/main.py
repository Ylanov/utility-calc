...
# Импортируем все роутеры приложения
from app.routers import (
    ...
    admin_adjustments,
    financier,
    telegram_app  # <-- ДОБАВЛЕНО: Роутер для Telegram Mini App
)

# Импортируем роутеры для системы СТРОБ.Арсенал
...
# Инициализация Sentry для отслеживания ошибок (если указан DSN)
...
app = FastAPI(
    title="Utility Calculator & Arsenal",
    version="1.0.0",
...
# ДОБАВЛЕНО: Роутер Telegram Mini App
...
async def add_security_headers(request: Request, call_next):
    ...
    # Строгий HTTPS (HSTS) - включается только в production для защиты от перехвата трафика
...
async def startup_event():
    ...
    try:
        ...
        # Инициализация Rate Limiter (защита от DDoS и спам-запросов)
