...
async function apiFetch(url, options = {}) {
    ...
    try {
        ...
        if (response.status === 401) {
            window.location.href = 'gsm_login.html';
            return null;
        }
        return response;
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', async () => {
    // Проверка прав (Ролевая модель ГСМ)
    const userRole = localStorage.getItem('gsm_role');

    // unit_head в контексте ГСМ — это начальник склада ГСМ или резервуарного парка
...
function bindEvents() {
    ...
    // ============================================================
    // ПРИВЯЗКА КНОПОК В МОДАЛКЕ СОЗДАНИЯ ДОКУМЕНТА
    // ============================================================
    ...
    document.getElementById('logoutBtn')?.addEventListener('click', async () => {
        try { await fetch('/api/gsm/logout', { method: 'POST' }); } catch (e) {}
        window.location.href = 'gsm_login.html';
    });
}
...
function injectSourceSelectIfNeeded() {
    ...
    sourceContainer.innerHTML = `
        <label for="newDocSource" class="block text-gray-700 font-bold mb-1">Отправитель / Поставщик</label>
        <select id="newDocSource" class="w-full border border-gray-300 p-2 rounded bg-white focus:border-amber-500 outline-none">
...
async function loadDocuments() {
    ...
    tableBody.innerHTML = '<tr><td colspan="7" class="text-center p-8"><i class="fa-solid fa-spinner fa-spin text-amber-600"></i> Загрузка журнала...</td></tr>';
    try {
        const response = await apiFetch('/api/gsm/documents');
        ...
        documents.forEach(doc => {
            ...
            tableRow.className = "cursor-pointer hover:bg-amber-50 transition border-b";
            tableRow.onclick = (e) => { if (!e.target.closest('.delete-btn')) openViewDocModal(doc.id); };
            let icon = '<i class="fa-solid fa-file-lines text-gray-400"></i>';
            if (doc.type === 'Первичный ввод') icon = '<i class="fa-solid fa-truck-droplet text-green-600"></i>';
            else if (['Отправка', 'Перемещение', 'Прием'].includes(doc.type)) icon = '<i class="fa-solid fa-truck-arrow-right text-orange-600"></i>';
            else if (doc.type === 'Списание') icon = '<i class="fa-solid fa-fire text-red-600"></i>';

            tableRow.innerHTML = `
                <td class="text-center text-lg py-3">${icon}</td>
                <td class="text-sm">${doc.date}</td>
                <td class="font-bold text-amber-900 text-sm">${doc.doc_number}</td>
        ...
        tableBody.innerHTML = '<tr><td colspan="7" class="text-center text-red-500 p-4">Ошибка загрузки журнала ГСМ.</td></tr>';
    }
}
...
async function createDocument() {
    ...
    // В ГСМ номер часто авто, но проверка остается для совместимости
    const docNumber = document.getElementById('newDocNumber').value;

    if (!docNumber) return alert('Введите номер документа.');
    if (docType === 'Первичный ввод' && !targetId) return alert('Укажите получателя (Резервуар/Склад).');
    if (docType === 'Списание' && !sourceId) return alert('Укажите источник (Откуда списываем).');
    ...
    document.querySelectorAll('#docItemsTable tbody tr').forEach(row => {
        ...
        const serial = row.querySelector('.serial-input').value; // В ГСМ это Партия/Паспорт
        const quantity = row.querySelector('.qty-input').value; // Объем
    ...
    if (!validationPassed) return alert('Укажите номер Паспорта качества или Партии для ГСМ.');
    if (items.length === 0) return alert('Добавьте позиции ГСМ в спецификацию.');
...
async function deleteDocument(id) {
    if (!confirm('Удалить документ движения ГСМ?')) return;
    try {
        const response = await apiFetch(`/api/gsm/documents/${id}`, { method: 'DELETE' });
...
async function openViewDocModal(id) {
    ...
    try {
        const response = await apiFetch(`/api/gsm/documents/${id}`);
        ...
        if (doc.items.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="3" class="text-center text-gray-500">Нет позиций.</td></tr>';
        }
        ...
        doc.items.forEach(item => {
            tableBody.innerHTML += `
                <tr class="border-b last:border-0">
                    ...
                    <td class="p-2 font-mono text-amber-700">${item.serial_number || '-'}</td>
                    <td class="p-2 text-center font-bold">${item.quantity}</td>
                </tr>`;
        });
    } catch (error) {
        tableBody.innerHTML = '<tr><td colspan="3" class="text-red-500 text-center">Ошибка загрузки.</td></tr>';
    }
}

// 2. ОБЪЕКТЫ (Резервуары, склады)
async function loadObjectsTree() {
    ...
    try {
        const response = await apiFetch('/api/gsm/objects');
        if (!response || !response.ok) return;
        objects = await response.json();
        if (objects.length === 0) {
            container.innerHTML = '<div class="p-4 text-sm text-gray-500">Нет объектов инфраструктуры ГСМ.</div>';
        ...
        container.innerHTML = objects.map(o => `
            <div class="tree-node pl-4 transition hover:bg-amber-50 flex justify-between items-center group">
                <div class="flex-grow cursor-pointer py-1" onclick="showBalanceModal(${o.id}, '${o.name}')">
                    <i class="fa-solid fa-database text-amber-500 mr-2"></i>
                    <span class="text-gray-700 ml-1 font-medium text-sm">${o.name}</span>
                    <span class="text-xs text-gray-400 ml-2">(${o.obj_type})</span>
                </div>
                <button onclick="showBalanceModal(${o.id}, '${o.name}')" class="text-gray-300 hover:text-green-600 px-2 py-1 text-xs opacity-0 group-hover:opacity-100 transition"><i class="fa-solid fa-chart-column"></i></button>
            </div>`).join('');
...
// 3. НОМЕНКЛАТУРА (Справочник ГСМ)
async function loadNomenclature() {
    try {
        const response = await apiFetch('/api/gsm/nomenclature');
        if (!response || !response.ok) return;
...
function renderNomenclatureList() {
    ...
    if (nomenclatures.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="2" class="p-4 text-center">Справочник ГСМ пуст.</td></tr>';
        return;
    }
    tableBody.innerHTML = nomenclatures.map(n => `
        <tr class="hover:bg-gray-100 border-b last:border-0">
            <td class="p-2 border-r font-mono text-xs text-amber-600">${n.code || '-'}</td>
...
function openNomenclatureModal() {
    ...
    document.getElementById('newNomIsNumbered').checked = false; // Для ГСМ по умолчанию налив (не фасовка)
...
async function createNomenclature() {
    ...
    const isNumbered = document.getElementById('newNomIsNumbered').checked; // Фасовка или Налив

    if (!name) return alert("Наименование марки ГСМ обязательно.");

    try {
        const response = await apiFetch('/api/gsm/nomenclature', {
            method: 'POST',
            body: JSON.stringify({ code, name, category, is_numbered: isNumbered })
        });
...
async function showBalanceModal(objectId, objectName) {
    ...
    title.innerText = `Остатки ГСМ: ${objectName}`;
    ...
    try {
        const response = await apiFetch(`/api/gsm/balance/${objectId}`);
        if (!response || !response.ok) throw new Error('Ошибка');
        const items = await response.json();

        if (items.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="4" class="text-center p-8 text-gray-500">Резервуар / Склад пуст.</td></tr>';
            return;
        }
        ...
        items.forEach(item => {
            tableBody.innerHTML += `
                <tr>
                    ...
                    <td class="p-2 border-b font-mono text-amber-700">${item.serial_number}</td>
                    <td class="p-2 border-b text-center font-bold bg-amber-50">${item.quantity}</td>
                </tr>`;
        });
    } catch (error) {
        tableBody.innerHTML = '<tr><td colspan="4" class="text-center p-8 text-red-500">Ошибка загрузки остатков.</td></tr>';
    }
}

// 5. ОТЧЕТЫ (ИСТОРИЯ ДВИЖЕНИЯ ПАРТИИ)
async function searchForReport() {
    const query = document.getElementById('reportSearchInput').value.trim();
    if (query.length < 2) return alert("Минимум 2 символа для поиска");
    ...
    try {
        // Эндпоинт для поиска партий
        const response = await apiFetch(`/api/gsm/reports/search?q=${encodeURIComponent(query)}`);
        const items = await response.json();
        listContainer.innerHTML = '';

        if (items.length === 0) {
            listContainer.innerHTML = '<span class="text-gray-400 text-sm">Партии/Паспорта не найдены</span>';
            return;
        }

        items.forEach(item => {
            const btn = document.createElement('button');
            btn.className = "text-sm border border-amber-200 bg-amber-50 hover:bg-amber-100 px-3 py-1 rounded text-amber-900 transition text-left";
            btn.innerHTML = `<b>${item.name}</b> <span class="text-xs text-gray-500">Паспорт: ${item.serial}</span>`;
...
async function loadTimeline(serial, nomId, name) {
    ...
    try {
        const response = await apiFetch(`/api/gsm/reports/timeline?serial=${encodeURIComponent(serial)}&nom_id=${nomId}`);
        const data = await response.json();

        statusBox.innerText = `${name} (Паспорт: ${serial}) — ${data.status}`;
        statusBox.classList.remove('hidden');
        container.innerHTML = '';

        if (data.history.length === 0) {
            container.innerHTML = '<div class="pl-6">История движения пуста.</div>';
            return;
        }
        ...
        data.history.forEach(event => {
            ...
            else if (event.op_type === "Списание") { color = "bg-red-500"; icon = "fa-fire"; }
            else if (event.op_type === "Перемещение") { color = "bg-blue-500"; icon = "fa-truck"; }
            else if (event.op_type === "Отправка") { color = "bg-amber-500"; icon = "fa-arrow-right"; }
                ...
                <div class="mb-6 ml-6 relative group">
                    ...
                    <div class="bg-white border border-gray-200 rounded p-3 shadow-sm hover:shadow-md transition">
                        <div class="flex justify-between mb-1">
                            <span class="text-sm font-bold text-gray-800">${event.op_type} <span class="text-amber-600">(${event.quantity})</span></span>
                            <span class="text-xs text-gray-500">${event.date}</span>
                        </div>
                        <div class="text-sm text-gray-600 mb-1">Документ: <span class="font-mono text-amber-600 font-bold">${event.doc_number}</span></div>
                        <div class="text-xs flex items-center gap-2 text-gray-500 bg-gray-50 p-2 rounded">
                            <span class="truncate max-w-[120px]">${event.source}</span>
                            <i class="fa-solid fa-arrow-right text-gray-300"></i>
                            <span class="font-bold text-gray-700 truncate max-w-[120px]">${event.target}</span>
                        </div>
                    </div>
                </div>`;
        });
    } catch (e) {
        container.innerHTML = `<div class="pl-6 text-red-500">Ошибка: ${e.message}</div>`;
    }
}
...
function addDocRow() {
    ...
    const opts = '<option value="">-- Выберите марку --</option>' + nomenclatures.map(n => `<option value="${n.id}" data-is-numbered="${n.is_numbered}">${n.name}${n.code ? ' ('+n.code+')' : ''}</option>`).join('');

    tableRow.innerHTML = `
        <td class="p-1"><select class="nom-select w-full border border-gray-300 p-1.5 rounded text-sm bg-white" onchange="handleNomenclatureChange(this)">${opts}</select></td>
        <td class="p-1"><input type="text" class="serial-input w-full border border-gray-300 p-1.5 rounded text-sm" placeholder="Паспорт / Партия"></td>
        <td class="p-1"><input type="number" step="0.01" class="qty-input w-full border border-gray-300 p-1.5 rounded text-sm text-center" value="0.00" min="0"></td>
...
function handleNomenclatureChange(el) {
    ...
    const isNum = el.options[el.selectedIndex].dataset.isNumbered === 'true'; // В ГСМ true = фасовка (бочки)
    ...
    if (isNum) {
        ...
        ser.placeholder = "Штрихкод / № тары";
    } else {
        qty.readOnly = false;
        qty.step = "0.01";
        qty.classList.remove('bg-gray-100');
        ser.placeholder = "Паспорт качества / Партия";
    }
}
...
function showCredentialsModal(title, username, password) {
    ...
    copyBtn.className = "absolute top-4 right-4 bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-amber-600 px-3 py-1 rounded text-sm shadow-sm transition flex items-center gap-2";
...
async function loadAndShowUsers() {
    ...
    try {
        const response = await apiFetch('/api/gsm/users');
        ...
        users.forEach(u => {
            const roleBadge = u.role === 'admin'
                ...
                : '<span class="px-2 py-0.5 bg-blue-100 text-blue-800 rounded text-xs font-bold border border-blue-200">Начальник объекта ГСМ</span>';
...
async function resetUserPassword(userId, username) {
    ...
    try {
        const response = await apiFetch(`/api/gsm/users/${userId}/reset-password`, { method: 'POST' });
        if (response && response.ok) {
            const data = await response.json();
